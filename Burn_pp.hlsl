...
float4 main(PostProcessingInput input) : SV_Target
{
	...
	float burnLevelMax = /*gBurnHeight*/ 0+ glowAmount; 

	float3 outputColour;

	// Output black when current burn texture value below burning range
    if (burnTexture.r <= /*gBurnHeight*/0)
    {
    ...
    {
		outputColour = 0;/*FILTER, not 0, read the comment and refer to earlier post-processing shaders for code required*/
	}
	...
	else // Draw burning edges
	{
		...
		float glowLevel = 1.0f - (burnTexture.r - /*gBurnHeight*/0) / glowAmount;

		// Extract direction to crinkle (2D vector) from the g & b components of the burn texture sampled above
		float2 crinkleVector = 0/*FILTER, not 0, read the comment*/;
